...
internal class ChatDemo : ComposeContainer() {
            ...
            Column(
                ...
                Column(
                    ...
                    Row(
                        ...
                        val width = if (getPager().pageData.isAndroid) 320 else 300
                        TextField(
                            value = inputText,
    ...
    private fun chatContext(chatList: List<String>): JSONArray {
        val messages = JSONArray()
        chatList.forEachIndexed { index, msg ->
            if (msg.isNotBlank()) {
    ...
    private fun sendStreamMessage(chatList: MutableList<String>) {
        chatList.add("")
        var reply = ""
        val headers = JSONObject().apply {
            put("Authorization", "Bearer $CHAT_API_KEY")
            put("Content-Type", "application/json")
        }
        val param = JSONObject().apply {
            put("model", CHAT_MODEL)
            put("messages", chatContext(chatList))
            put("stream", true)
        }

        acquireModule<NetworkModule>(NetworkModule.MODULE_NAME)
            .httpRequestSSE(CHAT_URL, true, param, headers) { data, success, errorMsg, _ ->
                if (success) {
                    reply += extractContentFromDelta(data.optString("data"))
                    chatList[chatList.lastIndex] = reply
                } else {
                    chatList[chatList.lastIndex] = "Error: $errorMsg"
                }
            }
    }
    ...
    private fun extractContentFromDelta(delta: String): String {
        if (delta.startsWith("data: ")){
            val dataString = delta.removePrefix("data: ").trim()
            if (dataString != "[DONE]") {
